...
const PropertyPalDashboard = () => {
  ...
  // Fetch appliances data using API helper
  const fetchAppliancesData = async (propertyId) => {
    console.log(`Fetching appliances for property ID: ${propertyId}`);
    
    try {
      const appliancesData = await apiHelpers.get('/appliances/', { property_id: propertyId });
      
      if (Array.isArray(appliancesData)) {
        setAppliances(appliancesData);
      } else {
        console.error('Expected array of appliances but got:', appliancesData);
        setAppliances([]);
      }
    } catch (error) {
  ...
  // Fetch dashboard data from all services using API helpers
  const fetchDashboardData = async (propertyId) => {
    ...
    try {
      ...
      try {
        ...
        const maintenanceResponse = await apiHelpers.get('/maintenance/', {
          status: 'pending', 
          property_id: propertyId
        });
        
        if (Array.isArray(maintenanceResponse)) {
          setMaintenanceItems(maintenanceResponse);
        } else {
          console.error('Unexpected maintenance data format:', maintenanceResponse);
          setMaintenanceItems([]);
        }
      } catch (err) {
      ...
      try {
        ...
        const appliancesResponse = await apiHelpers.get('/appliances/', { 
          property_id: propertyId 
        });
        
        if (Array.isArray(appliancesResponse)) {
          setAppliances(appliancesResponse);
        } else {
          console.error('Unexpected appliances data format:', appliancesResponse);
          setAppliances([]);
        }
      } catch (err) {
      ...
      try {
        ...
        const documentsResponse = await apiHelpers.get('/documents/', { 
          property_id: propertyId 
        });
        
        if (Array.isArray(documentsResponse)) {
          setDocuments(documentsResponse.slice(0, 4));
        } else {
          console.error('Unexpected documents data format:', documentsResponse);
          setDocuments([]);
        }
      } catch (err) {
      ...
      try {
        ...
        const projectsResponse = await apiHelpers.get('/projects/', { 
          property_id: propertyId 
        });
        ...
        if (Array.isArray(projectsResponse)) {
          ...
          console.error('Unexpected projects data format:', projectsResponse);
          setProjects([]);
        }
      } catch (err) {
  ...
  // Fetch monthly expenses using API helpers
  const fetchMonthlyExpenses = async (propertyId) => {
    try {
      ...
      // Fetch expenses
      const expensesResponse = await apiHelpers.get('/finances/expenses/', {
        property_id: propertyId,
      ...
      const budgetResponse = await apiHelpers.get('/finances/budgets/', {
        property_id: propertyId,
        year: now.getFullYear(),
        month: now.getMonth() + 1
      });
      
      const budgets = Array.isArray(budgetResponse) ? budgetResponse : [];
  ...
  // Upload home photo using API helpers
  const uploadHomePhoto = async (file) => {
    ...
    try {
      ...
      try {
        const propertyResponse = await apiHelpers.get(`/properties/${currentProperty.id}`);
        
        if (propertyResponse) {
          // Update the current property with the new data
          setCurrentProperty(propertyResponse);
        }
      } catch (propertyErr) {
      ...
      if (response && response.url) {
        ...
        const newPhoto = {
          ...
          url: getFormattedImageUrl(response.url),
          is_primary: true,
      ...
      setError(err.message || 'Failed to upload photo. Please try again later.');
  ...
  // Set a photo as primary using API helpers
  ...
  // Delete a photo using API helpers
  ...
  // Fetch home photos using API helpers
  const fetchHomePhotos = async (propertyId) => {
    try {
      ...
      // Format the photo URLs
      const photosWithFullUrl = photos.map(photo => ({
        ...photo,
        url: getFormattedImageUrl(photo.url)
      }));
      ...
      >
        <div className="container mx-auto px-4">
              ...
              <div className="h-64 bg-gray-700 flex items-center justify-center">
                {homePhotos.length > 0 ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    {!currentProperty && (
                      <p className="text-sm text-red-400 mt-2">Please add a property first</p>
                    )}
                    {currentProperty && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="card p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">Appliance & Warranty Tracking</h2>
          ...
          {/* Add Property CTA (shown when no property exists) */}
          {properties.length === 0 && (
